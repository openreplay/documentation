...
metaDescription: "Options de configuration pour la méthode constructeur du traqueur"
---
...
<Aside type="tip">
Si vous utilisez un framework web particulier, consultez notre [section Configuration du Traqueur](/using-or) pour savoir comment commencer.
</Aside>
...
Consultez [Canvas et WebGL](/installation/canvas) pour savoir comment activer cette fonctionnalité et obtenir plus de détails sur les options disponibles.
...
Consultez [Suivi des iFrames inter-domaines](/tutorials/crossdomain-iframe) pour plus de détails sur la façon de capturer un iFrame provenant d'un domaine différent.
...
<Aside type="caution">
    Notez que lors de l'utilisation de `__DISABLE_SECURE_MODE` depuis votre poste de travail local, vous **pourriez** voir des différences visuelles lors de la visualisation de la relecture, comme des styles CSS ne se chargeant pas correctement.
