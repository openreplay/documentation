...
<Aside type="tip">
  Nous proposons également un tutoriel détaillé sur la façon d'utiliser le plugin GraphQL pour capturer et modifier les requêtes, vous pouvez le consulter [ici](/tutorials/graphql).
</Aside>
...
Pour [Apollo](https://www.apollographql.com/), vous devriez créer un nouvel `ApolloLink` :
...
## Tutoriel
Si vous recherchez un exemple pratique d'utilisation de ce plugin pour capturer les données d'opérations (mutations et requêtes) dans vos relectures de sessions, consultez [notre tutoriel détaillé ici](/tutorials/graphql).
