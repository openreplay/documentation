...
| groups | Représente le rôle et le privilège d'administrateur de l'utilisateur dans OpenReplay ; selon votre fournisseur d'identité, il peut s'agir de `user.role` ou de `group`, etc. (facultatif, valeur par défaut = `member` sans privilège d'administrateur). Pour simplifier la gestion des rôles OpenReplay du côté de l'IdP, vous pouvez créer un groupe appelé OpenReplay-admin où vous placez les utilisateurs disposant du privilège d'administrateur, et un groupe appelé OpenReplay-member où vous placez les utilisateurs ayant le rôle 'member' |
...
3. Définissez le 'Display Label' sur OpenReplay (vous pouvez téléverser cette <a href="https://docs.openreplay.com/static/OpenReplay150x150.png" target="_blank">icône</a> pour votre application) puis accédez à l'onglet 'SSO'
4. Définissez :
...
- **IDP URL** afin qu'elle se termine par <code>openreplay-<ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> (cette valeur ne peut pas être modifiée ultérieurement ; si vous utilisez notre offre Cloud Dedicated, laissez la partie tenantKey vide)
