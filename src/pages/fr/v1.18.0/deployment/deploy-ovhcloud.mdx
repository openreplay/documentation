...
La stack OpenReplay peut être installée sur une seule machine, et un serveur dédié, un VPS, une machine virtuelle Private Cloud ou une instance Public Cloud OVHcloud constitue un candidat idéal. Voici comment procéder.
...
1. Tout d'abord, rendez-vous dans votre espace client OVHcloud sous 'Web cloud' > 'Domain names' > votre domaine (c'est-à-dire mycompany.com) > 'DNS zone' (ou votre autre fournisseur de services DNS) et créez un `A Record`. Utilisez le domaine que vous avez fourni précédemment lors de l'étape d'installation et faites-le pointer vers le serveur/instance en utilisant son IP publique.
...
Si vous rencontrez des problèmes, rejoignez notre [communauté Slack](https://slack.openreplay.com/) et obtenez de l'aide auprès de nos développeurs.
