...
<Aside type="tip">
**Note pour les utilisateurs auto-hébergés** : si vous utilisez la version auto-hébergée d'OpenReplay, vous devrez également spécifier la propriété de configuration `ingestPoint`. 
Cette propriété spécifie le point de terminaison d'ingestion pour les données du tracker. Les utilisateurs du Cloud n'en ont pas besoin, 
...
À noter, la `projectKey` que vous configurez est celle fournie par la plateforme OpenReplay. Pour des raisons de sécurité, cette valeur ne doit pas être stockée en dur dans votre code, mais plutôt dans une sorte de fichier/système de configuration.
