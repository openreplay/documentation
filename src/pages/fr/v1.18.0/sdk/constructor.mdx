...
- `disableClickmaps?: boolean` Pour désactiver le calcul des sélecteurs CSS (utilisés dans les cartes de clics). Par défaut : `false`.
...
- `assistSocketHost?: string` Peut être utilisé pour définir un hôte spécifique pour la connexion du plugin assist. Par défaut, il prend la valeur de `ingestPoint`.
...
Consultez [Canvas et WebGL](/installation/canvas) pour savoir comment activer cette capacité et obtenir plus de détails sur les options disponibles.
...
Consultez [Suivi des iFrames inter-domaines](tutorials/crossdomain) pour plus de détails sur la façon de capturer un iFrame d'un domaine différent.
