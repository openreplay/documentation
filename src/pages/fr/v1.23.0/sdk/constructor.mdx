...
Si votre site web est une **Single Page Application (SPA)**, vous pouvez utiliser le tracker en tant que singleton :
...
Sinon, si votre application web est **Server-Side-Rendered (SSR)** (c'est-à-dire NextJS, NuxtJS), utilisez le snippet ci-dessous.
Assurez-vous que `tracker.start()` est appelé une fois que l'application est démarrée (dans `useEffect` ou `componentDidMount`).
...
- `assistSocketHost?: string` Peut être utilisé pour définir un hôte spécifique pour la connexion du plugin assist. Par défaut, prend la valeur de `ingestPoint`.
...
- `inlineCss: number` : Force le tracker à enregistrer et envoyer les feuilles de style distantes comme des messages normaux au lieu de les mettre en cache côté backend. Ceci est utile lorsque les fichiers CSS sont privés et ne peuvent donc pas être mis en cache par le backend OpenReplay. Les valeurs peuvent être :
    ...
    - `3` : Récupère le fichier, puis l'enregistre en CSS brut dans un nœud `<style>` -- utilisez cette option si vous avez beaucoup de propriétés raccourcies dans vos fichiers CSS (couvre un bug de la spécification CSSOM).
- `css: CssRulesOptions` : Offre un meilleur support pour les applications construites avec la bibliothèque emotion-js (c'est-à-dire les composants MUI). Elle comprend les paramètres suivants :
    - `scanInMemoryCSS: boolean` : Activez-le si vous avez des problèmes avec les styles créés par emotionjs. Par défaut : `false`.
    ...
    - `checkLimit: number` : Utile pour les cas avec un nombre limité de mutations et/ou lorsque les feuilles de style sont hydratées sur le client après le rendu initial. Appliqué à chaque feuille de style individuellement. La valeur devrait être = X/checkCssInterval où X est la durée d'observation en ms. Par défaut : `0` (désactivé). 
...
- `obscureInputNumbers?: boolean` Obscurcit les nombres dans les champs de saisie. Les valeurs numériques seront converties en une chaîne aléatoire d'astérisques. Si désactivé, assurez-vous d'[assainir les champs de saisie](/installation/sanitize-data/#granular-level) qui peuvent contenir des données sensibles telles que des codes postaux ou des détails de cartes de crédit. Par défaut : `true`.
...
Notez que les données exclues sont obscurcies ou supprimées avant d'être envoyées aux serveurs OpenReplay. Les modifications appliquées aux options ci-dessus ne peuvent pas être rétroactives et ne s'appliqueront qu'aux nouvelles données collectées. Voir [Sanitize Data](/installation/sanitize-data) pour plus de détails.
...
Voir [Network Options](/sdk/network-options) pour des exemples et plus de détails sur la façon de masquer les données sensibles. 
...
Voir [Canvas and WebGL](/session-replay/canvas) pour savoir comment activer cette fonctionnalité et plus de détails sur les options disponibles.
...
Voir [Cross-domain iFrame Tracking](/session-replay/crossdomain-iframe) pour plus de détails sur la façon de capturer un iFrame d'un domaine différent. 
