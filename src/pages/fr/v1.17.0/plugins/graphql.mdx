...
Pour [Relay](https://relay.dev/), vous devez placer manuellement l'appel à `recordGraphQL` dans l'implémentation de `NetworkLayer`. Si vous utilisez la méthode standard `Network.create` pour l'implémenter, vous devez alors suivre l'exemple ci-dessous.
...
Pour [Apollo](https://www.apollographql.com/), vous devez créer un nouveau `ApolloLink` avec l'appel à `recordGraphQL` et l'ajouter à votre chaîne. Voici un exemple de la façon de procéder.
...
Consultez [Apollo Link](https://www.apollographql.com/docs/link/) et
[Apollo Networking](https://www.apollographql.com/docs/react/networking/network-layer/)
pour plus de détails.

## Tutoriel
Si vous recherchez un exemple pratique de l'utilisation de ce plugin pour capturer les données d'opérations (mutations et requêtes) dans vos relectures de sessions, consultez [notre tutoriel détaillé ici](/tutorials/graphql).
