...
Dans le cas de Redux, OpenReplay [fournit un plugin](https://docs.openreplay.com/plugins/redux) qui vous permet de vous intégrer au fonctionnement interne du store. Ce plugin vous laissera voir l'état (le « state ») du store Redux et les actions dispatchées tout au long de la session enregistrée.
...
Pour ce tutoriel, nous utiliserons [ce dépôt](https://github.com/deleteman/nextjs-commerce-example/tree/redux-store) (branche **redux-store**) d'un site de commerce électronique générique construit avec Next.js.

Dans ce projet, nous remplacerons un ensemble de produits mis en avant par un ensemble de nouveaux produits provenant d'une API externe.
...
**Remarque :** Il s'agit d'une application Next.js complexe, elle peut donc ne pas suivre la structure standard que l'on trouve dans les applications de listes de tâches (To-Do) classiques, mais en suivant ce tutoriel, vous devriez pouvoir suivre les changements.
...
Ceci fait, créez un dossier nommé `store` à la racine de votre projet et reproduisez la structure suivante :
...
## Configurer le fournisseur (provider) du tracker
...
Ce fournisseur vous permettra de configurer un ensemble de plugins ; dans notre cas, nous utiliserons le plugin Redux, de la manière suivante (depuis le fichier `_app.tsx`)
...
Maintenant, ce code vous permet de configurer le tracker avec le bon plugin, mais pour que le plugin fonctionne, nous devrons accéder au middleware renvoyé lorsque le plugin est appelé. Cela signifie que nous devrons garder une trace des valeurs renvoyées par nos plugins afin de pouvoir les utiliser ailleurs. Dans ce cas, nous en aurons besoin lors de l'appel de la fonction `createReduxStore` mentionnée précédemment.

Pour ce faire, nous devons étendre le `TrackerProvider` afin de nous assurer de conserver la valeur renvoyée dans l'état, comme ceci (vous pouvez consulter la version complète de ce fichier [ici](https://github.com/deleteman/nextjs-commerce-example/blob/redux-store/site/context/trackerProvider.js)) :
...
Et avec cela, vous devriez pouvoir déployer votre application (à condition d'avoir configuré la clé d'API comme indiqué [dans ce tutoriel](https://docs.openreplay.com/tutorials/next))
...
Si vous rencontrez des problèmes lors de la configuration du plugin Redux, n'hésitez pas à nous contacter sur notre [communauté Slack](https://slack.openreplay.com/) et à poser directement vos questions à nos développeurs !
