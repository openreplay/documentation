...
<Aside type="caution">
	La capture des payloads HTTP peut enregistrer des données hautement sensibles (c.-à-d. des mots de passe) ou des informations privées que votre application pourrait envoyer/recevoir dans les requêtes/réponses Fetch/XHR, alors assurez-vous d'assainir ces données à l'aide de l'option `sanitizer` ci-dessous.
</Aside>
...
Vous rencontrez des difficultés pour configurer ces options ? Connectez-vous à notre [Slack](https://slack.openreplay.com) ou consultez notre [Forum](https://forum.openreplay.com) et obtenez de l'aide de notre communauté.
