...
| ender | Met fin aux sessions utilisateur en cas d'inactivité ou de déconnexion |
| chalice | API permettant de servir le frontend |
...
Il est possible d'augmenter la capacité de n'importe quel service en remplaçant les valeurs d'allocation cpu/mémoire par défaut. Ces dernières sont déterminées lors du processus d'installation en fonction de la capacité de votre instance et devraient convenir aux besoins de la plupart des installations.
 ...
 Si vous avez un volume élevé et une machine puissante, exécutez simplement le fichier `openreplay -e` et remplacez les ressources du service. Dans l'exemple ci-dessous, nous le faisons pour le worker `http`. Mais cela peut être fait pour n'importe quel service (c'est-à-dire sink, storage, postgresql, redis, etc.) en décommentant et en mettant à jour le bloc ci-dessous. Si vous devez le faire pour plus d'un service, copiez et renommez/mettez à jour simplement le même bloc (attention aux doublons).
...
Vous trouverez ci-dessous les instructions pour mettre à niveau PostgreSQL vers la version 1.16 ou ultérieure :
...
1. Effectuez le dump de la base de données dans le volume persistant :
   ...
   > Remarque : Vous pouvez vous attendre à ce que les données soient compressées selon un ratio de 3:1 (exemple : si vous disposez de 15G de données postgres, elles occuperont 5G dans le dump). Assurez-vous donc d'avoir suffisamment d'espace disque.
...
3. Mettez à l'échelle le StatefulSet PostgreSQL à zéro pour arrêter les pods actuels :
...
5. Remettez le StatefulSet à l'échelle à un pour démarrer le pod mis à jour :
