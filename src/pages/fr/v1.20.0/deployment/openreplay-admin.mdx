...
| chalice | API servant le frontend |
...
1. Effectuez le dump de la base de données dans le volume persistant :
   ...
   Si vous utilisez une installation vanilla, exécutez la commande suivante pour effectuer un dump de la base de données :

   > Remarque : vous pouvez vous attendre à ce que les données soient compressées selon un ratio de 3:1 (exemple : si vous avez 15G de données postgres, elles feront 5G dans le dump). Assurez-vous donc de disposer de suffisamment d'espace disque.
...
3. Réduisez le StatefulSet PostgreSQL à zéro pour arrêter les pods actuels :

   ```sh
   kubectl scale sts postgresql --replicas=0 -n db
   ```

4. Mettez à jour l'image PostgreSQL vers la version 16.3.0 ou ultérieure :

   ```sh
   kubectl set image statefulset/postgresql postgresql=bitnamilegacy/postgresql:17.2.0 -n db
   ```

5. Remettez le StatefulSet à un pour démarrer le pod mis à jour :
