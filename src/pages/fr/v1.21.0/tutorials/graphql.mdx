...
metaTitle: "Utilisation du plugin GraphQL pour capturer et assainir les informations des opérations"
metaDescription: "Apprenez à configurer le plugin GraphQL dans votre application React"
---
Si vous utilisez GraphQL comme langage de requête pour vos API depuis votre application, alors le [plugin GraphQL](https://docs.openreplay.com/plugins/graphql) peut vous aider à suivre à la fois les mutations et les requêtes effectuées sur votre serveur GraphQL.
...
Avant d’installer le plugin GraphQL, vous devrez avoir installé le Tracker. Si vous savez déjà comment faire, passez à la section suivante ; sinon, continuez votre lecture.
...
Pour ce tutoriel, nous utiliserons la bibliothèque Apollo Boost, qui vous permet de modifier le flux de données de chaque requête grâce à ce qu’ils appellent des « links ».
...
Cela étant réglé, nous pouvons utiliser le link nouvellement créé de la manière suivante :
...
![L’interface GraphQL dans la Relecture de Session](images/graphql-ui.png)
...
Si je ne modifie pas les données, alors tout est enregistré sans changement. Donc, pour assainir les données dans la relecture **tout en gardant l’opération inchangée**, nous devons cloner les variables clés avant d’appeler le tracker. Cela signifie cloner les variables et les résultats de l’opération, et c’est tout ce que nous voulons. 

Voici donc un extrait de code qui créera l’ApolloLink et gardera les données secrètes dans les données de la relecture :
...
2. À l’intérieur du callback de `map` (de la fonction du link), nous ne renvoyons désormais plus la sortie de graphqlTracker, car cette fonction renverra telle quelle la valeur de résultat qu’elle a reçue. 1. Mais ce résultat sera renvoyé à l’application cliente, et si nous assainissons le résultat, l’utilisateur verra la version assainie du jeu de données. À la place, nous devons cloner le résultat pour modifier celui qui est suivi et renvoyer l’original.
...
Vous pouvez [consulter ce dépôt](https://github.com/deleteman/openreplay-graphql-example) pour le **code source complet** d’une application fonctionnelle basée sur GraphQL avec le Tracker.

Si vous rencontrez des problèmes pour configurer le Tracker sur votre projet GraphQL, veuillez nous contacter sur notre [communauté Slack](https://slack.openreplay.com/) et poser directement la question à nos développeurs !