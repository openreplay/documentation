...
Le déploiement d'OpenReplay repose sur les [Helm Charts](https://helm.sh), ce qui le rend entièrement compatible avec Kubernetes. Nous empaquetons PostgreSQL et Redis, qui sont nécessaires à OpenReplay. Voici comment procéder.
...
Le déploiement d'OpenReplay sur Kubernetes nécessite :
 - Kubernetes **v1.18+**
 - helm **3.10+**
 - **un PVC RWX avec la permission RW pour l'id utilisateur 1001** (pour les composants partagés, si le cluster comporte plus d'un nœud). Assurez-vous de remplacer `hostPath` par le nom du pvc créé dans `/var/lib/openreplay/vars.yaml`.
...
S'il s'agit d'un seul nœud, nous utiliserons hostVolume, qui est la valeur par défaut pour l'installation community.

Ensuite, ouvrez le fichier `vars.yaml` avec la commande `vim vars.yaml` puis remplacez :
- `domainName` : c'est l'endroit où OpenReplay sera accessible (c.-à-d. openreplay.mycompany.com)
...
OpenReplay traite des données utilisateur sensibles et nécessite donc HTTPS pour fonctionner. Ceci est obligatoire, sinon le tracker ne commencerait tout simplement pas à enregistrer. Il en va de même pour le tableau de bord : sans HTTPS, vous ne pourrez pas rejouer les sessions des utilisateurs.
...
L'un des principaux avantages d'exécuter OpenReplay derrière un load balancer cloud est de faire gérer le certificat par le fournisseur cloud. Vous trouverez ci-dessous des guides étape par étape sur la façon de créer :
...
Ensuite, assurez-vous que votre cluster provisionne un [type de service](https://kubernetes.io/docs/concepts/services-networking/service/#loadbalancer) `LoadBalancer` afin que le trafic provenant du load balancer cloud puisse être dirigé vers le backend d'OpenReplay. Maintenant, rendez-vous chez votre fournisseur de service DNS et créez un `A Record` qui pointe vers le cluster en utilisant son IP publique.
...
Comme alternative à la création d'un load balancer, vous pouvez apporter (ou générer) votre propre certificat SSL.
...
N'hésitez pas à nous le faire savoir sur Slack ou via une Github Issue si vous rencontrez des difficultés sur votre plateforme. Nous serons ravis de vous aider à le faire fonctionner.
