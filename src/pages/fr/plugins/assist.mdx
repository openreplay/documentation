...
L'approbation est mémorisée par onglet (dans `sessionStorage`) jusqu'à ce que [`stop()`](#méthodes) soit appelé, ce qui la révoque, de sorte que le prochain [`start()`](#méthodes) redemandera.
...
- `onSessionConfirmApprove?: (agentInfo: Record<string, any>) => any;` : Cette fonction de callback est déclenchée lorsque l'utilisateur approuve la visualisation de la session (en mode [`requestConfirm`](#widget-de-vue-en-direct)) et renvoie les détails de l'agent (email, nom et query).
...
- `onSessionConfirmDeny?: (agentInfo: Record<string, any>) => any;` : Cette fonction de callback est déclenchée lorsque l'utilisateur refuse la visualisation de la session (en mode [`requestConfirm`](#widget-de-vue-en-direct)) et renvoie les détails de l'agent (email, nom et query).
