---
title: "Événements"
metaTitle: "SDK Analytics OpenReplay — Événements"
...
Pour en savoir plus sur la gestion des événements dans l'interface d'OpenReplay, consultez [Data Management](/product-analytics/data-management).
...
Deux ensembles de clés différents sont rejetés, selon le helper :

**Propriétés par défaut** : collectées par le SDK lui-même et envoyées avec chaque événement. `setProperty` les ignore silencieusement, vous ne pouvez donc pas les masquer avec une super propriété :
...
**Clés de payload** : `properties`, `token` et `timestamp`. Elles font partie de l'enveloppe de l'événement, c'est pourquoi `setPropertiesOnce` et `unsetProperties` les ignorent.

<Aside type="note">
	Les deux listes sont appliquées par des méthodes différentes : `setProperty` protège les propriétés par défaut, tandis que `setPropertiesOnce` et `unsetProperties` protègent les clés de payload. La même répartition s'applique au module People, où `setProperties`, `setPropertiesOnce` et `appendValues` protègent tous les propriétés par défaut.
</Aside>
...
- Les [propriétés par défaut](#clés-réservées) collectées par le SDK sont ignorées et ne seront pas ajoutées en tant que super propriétés.
- La forme clé/valeur est ignorée lorsque `value` vaut `undefined` ; passez `null` pour stocker une valeur vide.
