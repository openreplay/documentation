---
title: "People"
metaTitle: "SDK Analytics d'OpenReplay — People"
metaDescription: "Identifiez les utilisateurs et gérez les propriétés utilisateur avec le module People d'analytics du tracker OpenReplay."
---
...
Pour en savoir plus sur la gestion des données utilisateur dans l'interface d'OpenReplay, consultez [Gestion des données](/product-analytics/data-management).
...
Identifie l'utilisateur actuel par un id de type chaîne (e-mail, nom d'utilisateur, id interne, etc.) et met en file d'attente un événement d'identité.
...
`user_id` doit être une chaîne non vide ; toute autre valeur lève une exception.

`options.fromTracker` est utilisé en interne pour éviter de déclencher deux fois le hook d'id au niveau du tracker ; en général, vous n'en avez pas besoin. `tracker.setUserID()` appelle déjà `identify` pour vous.

## reset

Réinitialise l'id de l'utilisateur actuel et toutes les propriétés people locales.
...
Si aucun utilisateur n'est identifié, l'opération est sans effet.
...
Les [propriétés par défaut](/sdk/analytics/events#clés-réservées) collectées par le SDK sont ignorées pour le cache local du SDK.

La forme clé/valeur exige que les deux arguments soient truthy : `setProperties("plan", "")` ou `setProperties("seats", 0)` lève l'erreur `invalid user properties provided to set`. Utilisez la forme objet pour les valeurs falsy.

Un événement est mis en file d'attente avec la charge utile `properties` complète que vous transmettez.

## setPropertiesOnce

Définit les propriétés uniquement si elles n'existent pas encore (du point de vue local du SDK).
...
Si la propriété n'existe pas encore localement, cet appel ne fait rien (retour anticipé). Définissez-la d'abord via `setProperties` / `setPropertiesOnce`.
...
Si la propriété n'existe pas localement, elle est initialisée à `0` avant l'incrémentation.
...
Gardez à l'esprit qu'à partir de la version 1.24.0, les sessions OpenReplay ne prennent en charge qu'un seul utilisateur, ce qui signifie que le nouvel utilisateur écrase le précédent dans la session.
