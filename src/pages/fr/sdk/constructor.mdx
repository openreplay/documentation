...
<Aside type="tip">
Si vous utilisez un framework web particulier, consultez notre section [Tracker Setup](/sdk/using-or) pour savoir comment commencer.
</Aside>
...
- `analytics?: { active: boolean, ingestPoint?: string }` Active le [SDK d'analyse](/sdk/analytics), qui expose `tracker.analytics` (événements, super propriétés et profils utilisateur) ainsi que le raccourci de premier niveau `tracker.track()`. Le SDK n'est **pas** instancié tant que `active` ne vaut pas `true`, et `tracker.analytics` reste `null` jusque-là. `ingestPoint` permet aux déploiements auto-hébergés d'envoyer les données d'analyse vers un point de terminaison différent de celui d'ingestion des replays ; il est ignoré sur OpenReplay Cloud. Valeur par défaut : `{ active: false }`.
...
- `obscureInputNumbers?: boolean` Masque les nombres dans les champs de saisie. Les valeurs numériques seront converties en une chaîne aléatoire d'astérisques. Si cette option est désactivée, veillez à [assainir les champs de saisie](/sdk/sanitize-data#niveau-granulaire) susceptibles de contenir des données sensibles telles que des codes postaux ou des coordonnées de carte de crédit. Valeur par défaut : `true`.
...
<a class="or-card2 or-card-wide" href="/sdk/methods"><span class="or-card-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m8 8-4 4 4 4M16 8l4 4-4 4M14 5l-4 14"/></svg></span><div><h4>Découvrez les méthodes</h4></div></a>
