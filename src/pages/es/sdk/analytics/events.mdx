---
title: "Eventos"
metaTitle: "SDK de Analítica de OpenReplay — Eventos"
metaDescription: "Define y gestiona super propiedades para los eventos de analítica del tracker de OpenReplay."
---
...
Para obtener más información sobre la gestión de eventos en la interfaz de OpenReplay, visita [Data Management](/product-analytics/data-management).
...
Se rechazan dos conjuntos distintos de claves, según el helper:

**Propiedades predeterminadas**: las recopila el propio SDK y se envían con cada evento. `setProperty` las omite silenciosamente, por lo que no puedes sobrescribirlas con una super propiedad:
...
**Claves del payload**: `properties`, `token` y `timestamp`. Forman parte del envoltorio del evento, por lo que `setPropertiesOnce` y `unsetProperties` las omiten.

<Aside type="note">
	Las dos listas las aplican métodos distintos: `setProperty` protege las propiedades predeterminadas, y `setPropertiesOnce` y `unsetProperties` protegen las claves del payload. La misma división se aplica en el módulo People, donde `setProperties`, `setPropertiesOnce` y `appendValues` protegen todas las propiedades predeterminadas.
</Aside>
...
- Las [propiedades predeterminadas](#claves-reservadas) recopiladas por el SDK se ignoran y no se añadirán como super propiedades.
- La forma clave/valor se ignora cuando `value` es `undefined`; pasa `null` para almacenar un valor vacío.
