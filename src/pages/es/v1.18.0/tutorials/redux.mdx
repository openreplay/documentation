...
metaDescription: "Aprende a capturar cambios de estado utilizando el plugin de Redux y a visualizarlos en las repeticiones de sesión"
---
import Aside from '~/components/Aside.astro'
import YoutubeVideo from '~/components/YoutubeVideo.astro'

<YoutubeVideo title="Mira cómo rastrear el estado con Redux"
  description="Si no te gusta leer, puedes seguir este tutorial en video que te muestra cómo rastrear el estado de tus aplicaciones React cuando usas Redux"
  videoURL="https://www.youtube.com/watch?v=vSg4_i-jCj0"
  />

Si necesitas más visibilidad al reproducir las sesiones de tus usuarios, echar un vistazo al estado de la aplicación puede ser muy útil.

En el caso de Redux, OpenReplay [proporciona un plugin](https://docs.openreplay.com/plugins/redux) que te permite integrarte con el funcionamiento interno del store. Este plugin te dejará ver el estado (state) del store de Redux y las acciones despachadas a lo largo de la sesión grabada.
...
## Configuración de Redux en un proyecto Next.js

Para este tutorial, usaremos [este repositorio](https://github.com/deleteman/nextjs-commerce-example/tree/redux-store) (rama **redux-store**) de un sitio genérico de comercio electrónico construido con Next.js.
...
**Nota:** Esta es una aplicación Next.js compleja, por lo que es posible que no siga la estructura estándar que se encuentra en las clásicas apps de tareas (To-Do), pero siguiendo este tutorial deberías poder seguir el ritmo de los cambios.

**Recuerda:** siempre puedes clonar el repositorio y revisar el código tú mismo.
...
El archivo `types.js` contendrá la definición de tipos para las dos acciones que vamos a definir:
...
Y finalmente, el archivo de acciones definirá una sola función, encargada de obtener la lista de productos desde una API externa y despachar la acción y el payload correctos:
...
## Configuración del proveedor del tracker
...
Este proveedor te permitirá configurar un conjunto de plugins; en nuestro caso, usaremos el plugin de Redux, así (desde dentro del archivo `_app.tsx`)
...
Ahora bien, este código te permite configurar el tracker con el plugin correcto, pero para que el plugin funcione, necesitaremos acceder al middleware devuelto cuando se llama al plugin. Eso significa que tendremos que llevar un registro de los valores devueltos por nuestros plugins para poder usarlos en otra parte. En este caso, necesitaremos usarlo al llamar a la función `createReduxStore` de arriba.

Para hacerlo, tenemos que extender el `TrackerProvider` para asegurarnos de conservar el valor devuelto dentro del estado, así (puedes revisar la versión completa de este archivo [aquí](https://github.com/deleteman/nextjs-commerce-example/blob/redux-store/site/context/trackerProvider.js)):
...
Esa lógica te permite usar el plugin al inicializar el tracker y luego acceder al middleware y usarlo más tarde.

## Creación del store de Redux con el nuevo middleware
...
El hook `useEffect` desencadenará la llamada a `startTracking` y la obtención de la nueva lista de productos de maquillaje al despachar la llamada a la función `getMakeUpProducts`.

Y con eso, deberías poder desplegar tu aplicación (siempre que hayas configurado la API key como se menciona [en este tutorial](https://docs.openreplay.com/tutorials/next))
...
Si tienes algún problema al configurar el plugin de Redux, ¡contáctanos en nuestra [comunidad de Slack](https://slack.openreplay.com/) y pregunta directamente a nuestros desarrolladores!
