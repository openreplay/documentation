...
| Campo | Valor |
|----------|-------------|
| tenantKey | <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> su valor se encuentra en el Dashboard, en 'Preferences' > 'Account' (Nota: debe ignorarse/dejarse vacío si usas una instancia dedicated/cloud) |
...
5. Define los siguientes campos en 'Attribute Statements':
 - **tenantKey**: formato `Basic` y establece el valor en <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, que se encuentra en el dashboard de OpenReplay en 'Preferences' > 'Account' (Nota: debe ignorarse/dejarse vacío si usas una instancia dedicated/cloud).
...
5. Añade las siguientes notificaciones (claims) en el bloque 'Attribute & Claims' y asegúrate de que el campo 'Namespace' quede vacío en cada una de ellas:
 - **tenantKey**: establece el valor en <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, que se encuentra en el dashboard de OpenReplay en 'Preferences' > 'Account' (Nota: debe ignorarse/dejarse vacío si usas una instancia dedicated/cloud).
...
11. En la sección `chalice > env`, descomenta y establece los siguientes atributos usando los valores del paso 4:
   ...
   - **idp_tenantKey**: <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> se encuentra en el dashboard de OpenReplay en 'Preferences' > 'Account' (Nota: debe ignorarse/dejarse vacío si usas una instancia dedicated/cloud).
...
- **adminPrivileges**: opcional, ignóralo si lo definiste en el paso anterior; establece el valor en `true` si quieres dar privilegios de administrador a los nuevos usuarios, `false` si no
