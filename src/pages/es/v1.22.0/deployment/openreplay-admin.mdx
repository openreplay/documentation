...
La CLI es útil para gestionar aspectos básicos de tu instancia de OpenReplay, como reiniciar o reinstalar un servicio, acceder a los registros de un componente o simplemente comprobar el estado de tus servicios de backend.
...
Por último, reinstala el servicio para que los nuevos límites surtan efecto (tus datos no se perderán):
...
A continuación se muestran las instrucciones para actualizar PostgreSQL a la versión 1.16 o posterior:

1. Realiza el volcado de la base de datos en el volumen persistente:

   Si utilizas una instalación estándar, ejecuta el siguiente comando para realizar un volcado de la base de datos:
...
5. Vuelve a escalar el StatefulSet a uno para iniciar el pod actualizado:
