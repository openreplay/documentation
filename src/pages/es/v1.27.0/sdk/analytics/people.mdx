...
`user_id` debe ser una cadena no vacía; cualquier otro valor lanza un error.

`options.fromTracker` se utiliza internamente para evitar llamar dos veces al hook de id a nivel del tracker; normalmente no lo necesitas. `tracker.setUserID()` ya llama a `identify` por ti.
...
Las [propiedades predeterminadas](/sdk/analytics/events#claves-reservadas) recopiladas por el SDK se ignoran para la caché local del SDK.

La forma clave/valor requiere que ambos argumentos sean truthy: `setProperties("plan", "")` o `setProperties("seats", 0)` lanza `invalid user properties provided to set`. Usa la forma de objeto para valores falsy.
