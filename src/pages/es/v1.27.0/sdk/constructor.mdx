...
De lo contrario, si tu aplicación web utiliza **renderizado del lado del servidor (SSR)** (es decir, NextJS, NuxtJS), usa el fragmento de código siguiente.
...
<Aside type="tip">
Si utilizas un framework web concreto, consulta nuestra sección [Tracker Setup](/sdk/using-or) para saber cómo empezar.
</Aside>
...
- `sessionHash?: string` El hash de la sesión inicial. Esto resulta útil cuando las sesiones recorren diferentes subdominios de tu aplicación web pero quieres unirlas en una sola grabación. En caso de que no sea posible continuar la sesión (no existe o ha finalizado), el tracker iniciará automáticamente una nueva. También se devuelve en `stop()`. Más detalles al respecto [aquí](/troubleshooting/session-recordings#las-reproducciones-se-rompen-entre-subdominios).
...
- `analytics?: { active: boolean, ingestPoint?: string }` Habilita el [SDK de analíticas](/sdk/analytics), que expone `tracker.analytics` (eventos, super propiedades y perfiles de usuario) además del atajo de nivel superior `tracker.track()`. El SDK **no** se instancia a menos que `active` sea `true`, y `tracker.analytics` sigue siendo `null` hasta entonces. `ingestPoint` permite que los despliegues self-hosted envíen las analíticas a un endpoint distinto del ingest de reproducción; se ignora en OpenReplay Cloud. Valor predeterminado: `{ active: false }`.
...
- `obscureInputNumbers?: boolean` Oculta los números en los campos de entrada. Los valores numéricos se convertirán en una cadena aleatoria de asteriscos. Si se desactiva, asegúrate de [sanear los campos de entrada](/sdk/sanitize-data#nivel-granular) que puedan contener datos sensibles, como códigos postales o datos de tarjetas de crédito. Valor predeterminado: `true`.
...
Ten en cuenta que los datos excluidos se ocultan o suprimen antes de enviarlos a los servidores de OpenReplay. Los cambios aplicados a las opciones anteriores no pueden ser retroactivos y solo se aplicarán a los datos recién recopilados. Consulta [Sanitize Data](/sdk/sanitize-data) para obtener más detalles.
...
Consulta [Network Options](/sdk/network-options) para ver ejemplos y obtener más detalles sobre cómo redactar datos sensibles.
...
```
A partir de la versión [14.0.10](https://github.com/openreplay/openreplay/blob/dev/tracker/tracker/CHANGELOG.md) del Tracker, **el atributo `data-domain` ya no es necesario** en los elementos HTML al trabajar con iFrames de dominios cruzados.

Consulta [Cross-domain iFrame Tracking](/session-replay/crossdomain-iframe) para obtener más detalles sobre cómo capturar un iFrame de un dominio diferente.
...
<Aside type="caution">
    Ten en cuenta que, al usar `__DISABLE_SECURE_MODE` desde tu estación de trabajo local, **es posible** que veas diferencias visuales al ver la reproducción, como estilos CSS que no se cargan correctamente. Sigue [esta guía](/troubleshooting/localhost) para solucionar la situación.
</Aside>

## Métodos

<a class="or-card2 or-card-wide" href="/sdk/methods"><span class="or-card-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m8 8-4 4 4 4M16 8l4 4-4 4M14 5l-4 14"/></svg></span><div><h4>Conoce los métodos</h4></div></a>
