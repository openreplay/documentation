...
| Campo | Valor |
|----------|-------------|
| tenantKey | <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> su valor se encuentra en el panel, en 'Preferences' > 'Account' (este atributo no debe añadirse si utilizas nuestro plan Cloud Dedicated) |
| firstName | `user.firstName` |
| lastName |  `user.lastName` |
| internalId | ID interno o `user.email` (el ID del usuario en tu sistema de gestión de identidades) (opcional) |
| groups | Representa el rol y los privilegios de administrador del usuario en OpenReplay; según tu proveedor de identidad puede ser `user.role` o `group`, etc. (opcional, por defecto = `member` sin privilegios de administrador). Para simplificar la gestión de roles de OpenReplay desde el IdP, puedes crear un grupo llamado OpenReplay-admin donde incluyas a los usuarios con privilegios de administrador, y un grupo llamado OpenReplay-member donde incluyas a los usuarios que tengan el rol 'member' |
...
| enforce_SSO | Si se establece en `true`, se bloquearán todas las conexiones con nombre de usuario y contraseña (opcional, por defecto=`false`) |
| idp_group_prefixes | Los prefijos del nombre de grupo del IdP (opcional, por defecto=`OpenReplay-,or-`) |
...
5. Define los siguientes campos en 'Attribute Statements':
 - **tenantKey**: formato `Basic` y establece el valor en <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, que se encuentra en el panel de OpenReplay en 'Preferences' > 'Account' (este atributo no debe añadirse si utilizas nuestro plan Cloud Dedicated)
...
5. Añade los siguientes claims en el bloque 'Attribute & Claims' y asegúrate de dejar vacío el campo 'Namespace' para cada uno de ellos:
 - **tenantKey**: establece el valor en <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, que se encuentra en el panel de OpenReplay en 'Preferences' > 'Account' (este atributo/claim no debe añadirse si utilizas nuestro plan Cloud Dedicated)
...
11. En la sección `chalice > env`, descomenta y establece los siguientes atributos usando los valores del paso 4:
   ...
   - **idp_tenantKey**: <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> que se encuentra en el panel de OpenReplay en 'Preferences' > 'Account'
...
4. Establece:
- **IdP Entity ID** en <code>openreplay/<ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> (<code>{initialValues.tenantKey}</code> se encuentra en el panel de OpenReplay en 'Preferences' > 'Account', pero si utilizas nuestro plan Cloud Dedicated, deja vacío el valor de tenantKey)
...
- **IDP URL** para que termine en <code>openreplay-<ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> (este valor no se puede cambiar más tarde; si utilizas nuestro plan Cloud Dedicated, deja vacía la parte de tenantKey)
...
- **tenantKey**: establece el valor en <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, que se encuentra en el panel de OpenReplay en 'Preferences' > 'Account' (este atributo no debe añadirse si utilizas nuestro plan Cloud Dedicated)
...
| Client id | <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> su valor se encuentra en el panel, en 'Preferences' > 'Account' |
