...
<Aside type="tip">
Если вы используете определённый веб-фреймворк, ознакомьтесь с нашим разделом [Tracker Setup](/sdk/using-or), чтобы узнать, как начать работу.
</Aside>
...
- `sessionHash?: string` Хеш начальной сессии. Это полезно, когда сессии проходят через разные субдомены вашего веб-приложения, но вы хотите объединить их в одну запись. Если продолжить сессию невозможно (она не существует или завершена), трекер автоматически начнёт новую. Это значение также возвращается при вызове `stop()`. Подробнее об этом [здесь](/troubleshooting/session-recordings#повторы-прерываются-между-поддоменами).
...
- `analytics?: { active: boolean, ingestPoint?: string }` Включает [SDK аналитики](/sdk/analytics), который предоставляет `tracker.analytics` (события, суперсвойства и профили пользователей), а также сокращённый метод верхнего уровня `tracker.track()`. SDK **не** создаётся, пока `active` не равен `true`, и до этого `tracker.analytics` остаётся `null`. `ingestPoint` позволяет self-hosted развёртываниям отправлять аналитику на эндпоинт, отличный от ingest для записей; в OpenReplay Cloud он игнорируется. По умолчанию: `{ active: false }`.
...
- `obscureInputNumbers?: boolean` Скрывает числа в полях ввода. Числовые значения будут преобразованы в случайную цепочку звёздочек. Если эта опция отключена, обязательно [очищайте поля ввода](/sdk/sanitize-data#детальный-уровень), которые могут содержать конфиденциальные данные, такие как почтовые индексы или реквизиты кредитной карты. По умолчанию: `true`.
...
Обратите внимание, что исключённые данные скрываются или подавляются до отправки данных на серверы OpenReplay. Изменения, применённые к указанным выше параметрам, не могут иметь обратной силы и будут применяться только к вновь собираемым данным. Подробнее см. в разделе [Sanitize Data](/sdk/sanitize-data).
