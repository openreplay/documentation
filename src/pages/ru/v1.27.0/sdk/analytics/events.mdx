---
title: "События"
metaTitle: "SDK аналитики OpenReplay — События"
metaDescription: "Определяйте и управляйте суперсвойствами для событий аналитики трекера OpenReplay."
---
import Aside from '~/components/Aside.astro'

## События

Этот модуль позволяет управлять **событиями** и **свойствами** — постоянными парами ключ/значение, которые автоматически добавляются к каждому отслеживаемому событию.

Суперсвойства хранятся локально и объединяются с полезной нагрузкой каждого события, когда вы отслеживаете события.

Чтобы узнать больше об управлении событиями в интерфейсе OpenReplay, посетите [Data Management](/product-analytics/data-management).

Доступно по адресу:

`tracker.analytics.events`

## Концепции

### Суперсвойства

Суперсвойства хранятся в SDK и объединяются с каждым событием в разделе `properties`.

### Свойства события

Свойства события передаются только для одного вызова `track/sendEvent` и **не сохраняются** как суперсвойства.

## Зарезервированные ключи

В зависимости от вспомогательной функции отклоняются два разных набора ключей:

**Свойства по умолчанию** — собираются самим SDK и отправляются с каждым событием. `setProperty` молча пропускает их, поэтому вы не можете перекрыть их суперсвойством:

`os`, `os_version`, `browser`, `browser_version`, `platform`, `screen_height`, `screen_width`, `initial_referrer`, `utm_source`, `utm_medium`, `utm_campaign`, `user_id`, `distinct_id`, `sdk_edition`, `sdk_version`, `timezone`, `search_engine`

**Ключи полезной нагрузки** — `properties`, `token` и `timestamp`. Они являются частью оболочки события, поэтому `setPropertiesOnce` и `unsetProperties` пропускают их.

<Aside type="note">
	Эти два списка контролируются разными методами: `setProperty` защищает свойства по умолчанию, а `setPropertiesOnce` и `unsetProperties` — ключи полезной нагрузки. То же разделение действует и в модуле People, где `setProperties`, `setPropertiesOnce` и `appendValues` защищают свойства по умолчанию.
</Aside>

## Сигнатуры верхнего уровня

```ts
// Track an event.
// The `properties` argument is per-event only (not persisted).
tracker.analytics.events.sendEvent(eventName: string, properties?: Record<string, any>, options?: { send_immediately: boolean })

// Create/update super properties (persisted)
tracker.analytics.events.setProperty(name: string, value: any): void
tracker.analytics.events.setProperty(properties: Record<string, any>): void

// Create super properties only if they don't exist yet (persisted)
tracker.analytics.events.setPropertiesOnce(name: string, value: any): void
tracker.analytics.events.setPropertiesOnce(properties: Record<string, any>): void

// Remove super properties (persisted)
tracker.analytics.events.unsetProperties(name: string): void
tracker.analytics.events.unsetProperties(names: string[]): void

// Clear all super properties
tracker.analytics.events.reset(): void
```

## Отслеживание событий со свойствами

### Свойства для отдельного события (не сохраняются)

```ts
tracker.analytics.events.sendEvent("Button Clicked", {
  button_text: "Start",
  page: "home",
})
```

Эти свойства применяются только к данному событию.

### Суперсвойства + свойства для отдельного события (объединённые)

Когда вы задаёте суперсвойства, они автоматически включаются в каждое событие, а затем объединяются со свойствами для отдельного события:

```ts
tracker.analytics.events.setProperty({
  app_version: "1.42.0",
  env: "prod",
})

tracker.analytics.events.sendEvent("Signup Started", {
	// per-event, can override a super property with the same key
  plan: "pro", 
})
```

Порядок объединения:

- суперсвойства добавляются первыми
- свойства для отдельного события объединяются поверх них

Таким образом, при коллизии ключей побеждают свойства для отдельного события.

## setProperty

Создаёт или перезаписывает суперсвойства.

```ts
// object form
tracker.analytics.events.setProperty({
  app_version: "1.42.0",
  region: "eu",
})

// key/value form
tracker.analytics.events.setProperty("build_sha", "3f2c9a1")
```

### Примечания:

- [Свойства по умолчанию](#зарезервированные-ключи), собираемые SDK, игнорируются и не будут добавлены как суперсвойства.
- Форма ключ/значение игнорируется, если `value` равно `undefined`; передайте `null`, чтобы сохранить пустое значение.
- Изменения сохраняются локально только в том случае, если что-то действительно изменилось.

## setPropertiesOnce

Создаёт суперсвойства только в том случае, если они ещё не существуют.

```ts
tracker.analytics.events.setPropertiesOnce({
  first_seen_at: Date.now(),
  signup_source: "docs",
})

// key/value form
tracker.analytics.events.setPropertiesOnce("initial_referrer", document.referrer)
```

### Примечания:

- Свойства задаются только в том случае, если ключ отсутствует.
- Зарезервированные ключи (`properties`, `token`, `timestamp`) игнорируются.

## unsetProperties

Удаляет одно или несколько суперсвойств.

```ts
// single
tracker.analytics.events.unsetProperties("region")

// multiple
tracker.analytics.events.unsetProperties(["app_version", "build_sha"])
```

### Примечания:

- Зарезервированные ключи удалить нельзя.
- Удаление сохраняется.

## reset

Очищает все суперсвойства.

```ts
tracker.analytics.events.reset()
```

## Отправить немедленно

По умолчанию события ставятся в очередь/группируются в пакеты. Вы можете запросить немедленную отправку события:

```ts
tracker.analytics.events.sendEvent(
  "Checkout Completed",
  { value: 199, currency: "EUR" },
  { send_immediately: true },
)
```

## Псевдонимы, совместимые с Mixpanel

Все методы поддерживают слой совместимости, предоставляемый для удобства перехода:

```ts
tracker.analytics.events.register       // setProperty
tracker.analytics.events.register_once  // setPropertiesOnce
tracker.analytics.events.unregister     // unsetProperties
tracker.analytics.events.track          // sendEvent
```


## Есть вопросы?

Если вы столкнётесь с какими-либо проблемами, присоединяйтесь к нашему [Slack](https://slack.openreplay.com) или загляните на наш [Форум](https://forum.openreplay.com) и получите помощь от нашего сообщества.
