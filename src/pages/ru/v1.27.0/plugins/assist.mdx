...
Разрешение запоминается для каждой вкладки (в `sessionStorage`) до вызова [`stop()`](#методы), который его отзывает, поэтому следующий вызов [`start()`](#методы) снова покажет запрос.
...
- `onSessionConfirmApprove?: (agentInfo: Record<string, any>) => any;`: Эта функция обратного вызова срабатывает, когда пользователь подтверждает просмотр сессии (в режиме [`requestConfirm`](#виджет-просмотра-в-реальном-времени)), и возвращает данные агента (email, имя и query).
...
- `onSessionConfirmDeny?: (agentInfo: Record<string, any>) => any;`: Эта функция обратного вызова срабатывает, когда пользователь отклоняет просмотр сессии (в режиме [`requestConfirm`](#виджет-просмотра-в-реальном-времени)), и возвращает данные агента (email, имя и query).
