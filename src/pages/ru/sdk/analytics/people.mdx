...
`user_id` должен быть непустой строкой — в любом другом случае выбрасывается исключение.

`options.fromTracker` используется внутренне, чтобы избежать двойного вызова хука id на уровне трекера; как правило, он вам не нужен. `tracker.setUserID()` уже вызывает `identify` за вас.
...
[Свойства по умолчанию](/sdk/analytics/events#зарезервированные-ключи), собираемые SDK, игнорируются для локального кэша SDK.

Форма ключ/значение требует, чтобы оба аргумента были truthy — `setProperties("plan", "")` или `setProperties("seats", 0)` выбрасывает `invalid user properties provided to set`. Для falsy-значений используйте объектную форму.
