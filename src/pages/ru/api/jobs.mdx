...
`Авторизация: {ВАШ КЛЮЧ ОРГАНИЗАЦИИ API}`: Ключ можно найти в «Настройках»> «Аккаунт»> «Ключ API организации».
...
Верните статус работы и другие детали, такие как `createdAt`, `referenceId` и `errors`.
...
Отмените работу, если она еще не началась или все еще в процессе.
...
`200`: Ответ отправлен в теле в формате JSON
`501`: Возвращается, если работу нельзя отменить
