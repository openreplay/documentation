...
**Примечание:** по умолчанию эта интеграция будет искать журналы в любом индексе, соответствующем шаблону `*log*`, если у вас есть определенный индекс для журналов, который не соответствует этому шаблону, пожалуйста, измените имя индекса в строке 12 в следующей команде. Если вы хотите указать более 1 индекса, вы можете разделить имена с помощью `,`.
...
Убедитесь, что скопировали `id` и `api_key`, потому что нам они нужны для нашей интеграции. Для получения дополнительной информации о создании ключа API, пожалуйста, обратитесь к этой документации.
...
Чтобы связать событие Elasticsearch с записанным сеансом пользователя, должен быть распространен уникальный токен из вашего фронтэнда в ваш бэкэнд при каждом запросе, который вы хотите отслеживать. Это можно сделать с помощью пользовательского заголовка HTTP. В приведенном ниже примере мы используем функцию `fetch` для отправки этого заголовка.
...
Для того, чтобы OpenReplay связал запись журнала Elasticsearch с записанным сеансом пользователя, должен быть распространен уникальный токен в качестве части каждой ошибки бэкэнда, которую вы хотите отслеживать.
