...
В противном случае, если ваше веб-приложение использует **Серверный рендеринг (SSR)** (например, NextJS, NuxtJS), используйте следующий код.
...
- `revID?: string` ID ревизии вашего веб-приложения. Полезно при поиске проблем, происходящих в определенной версии релиза.
...
- `inlineCss: number`: Заставляет трекер записывать и отправлять удалённые таблицы стилей как обычные сообщения вместо кэширования их на бэкенде. Это полезно, когда CSS-файлы являются приватными и поэтому не могут быть кэшированы бэкендом OpenReplay. Значения могут быть:
    ...
    - `2`: Получить файл, затем смоделировать поведение AdoptedStyleSheets для воспроизведения.
    - `3`: Получить файл, затем сохранить его как обычный css внутри узла `<style>` -- используйте эту опцию, если в ваших CSS-файлах много сокращённых свойств (покрывает баг спецификации CSSOM).
- `css: CssRulesOptions`: Обеспечивает лучшую поддержку приложений, построенных на библиотеке emotion-js (например, компонентов MUI). Имеет следующие параметры:
    - `scanInMemoryCSS: boolean`: Включите, если у вас есть проблемы со стилями, созданными emotionjs. По умолчанию: `false`.
    - `checkCssInterval: number`: Как часто сканировать отслеживаемые таблицы стилей с пустыми правилами. По умолчанию: `200` (мс).
    - `checkLimit: number`: Полезно для случаев с ограниченным количеством мутаций и/или когда таблицы стилей гидратируются на клиенте после первоначального рендеринга. Применяется к каждой таблице стилей индивидуально. Значение должно быть = X/checkCssInterval, где X — продолжительность наблюдения в мс. По умолчанию: `0` (отключено). 
...
- `urls`: Позволяет настроить, как записываются события местоположения страницы, с помощью следующих опций:
	- `urlSanitizer?: (url: string) => string` Позволяет очищать url страницы (для удаления токенов использования, например).
	- `titleSanitizer?: (title: string) => string` Позволяет очищать заголовок страницы.
- `resourceNameSanitizer?: (url: string) => string` Позволяет очищать URL для записанных сетевых ресурсов.
...
См. [Network Options](/sdk/network-options) для примеров и более подробной информации о том, как редактировать конфиденциальные данные. 
