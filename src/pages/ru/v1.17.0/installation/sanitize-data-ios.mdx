...
Очищенные представления будут размыты в итоговой записи. Вы можете добавить любой UI-компонент, и он будет автоматически удалён (для экономии памяти), как только перестанет отображаться на экране.
...
SDK экспортирует `ORSanitizedView`, который вы можете использовать, чтобы обернуть свои конфиденциальные компоненты и размыть их.
...
Просто назначьте список строк, которые вы хотели бы очистить:
