...
Ключ `graphqlTracker`, используемый здесь, может быть любым, что вы хотите. Пока ключ, используемый в разделе `plugins`, совпадает с тем, который вы извлекаете из результатов функции `init`, все будет хорошо.

## Настройка плагина с помощью клиента Apollo

Для этого учебника мы будем использовать библиотек
```jsx
