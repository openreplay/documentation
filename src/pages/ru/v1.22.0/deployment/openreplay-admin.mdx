...
CLI удобен для управления базовыми аспектами вашего экземпляра OpenReplay, такими как перезапуск или переустановка сервиса, доступ к журналам компонента или просто проверка состояния ваших backend-сервисов.
...
И ознакомьтесь со списком всех доступных опций:
...
| sink | Считывает данные из потокового конвейера (Redis или Kafka для редакции enterprise) и вставляет их во временное хранилище (NFS) |
| storage | Перемещает временные файлы записей сессий в сервис объектного хранилища |
| assets | Для кэширования ресурсов (CSS, шрифты и иконки) с целью корректного отображения записей |
| db | Считывает и записывает данные в различные базы данных (Postgres + ClickHouse для редакции enterprise) |
| ender | Завершает пользовательские сессии при бездействии или отключении |
| chalice | API для обслуживания frontend |
| alerts | Отправляет уведомления (электронная почта, slack, в приложении, webhook) при достижении заданного пользователем порога по любой из метрик производительности |
...
## Увеличение ёмкости сервиса

Можно увеличить ёмкость любого сервиса, переопределив значения выделения cpu/памяти по умолчанию. Последние определяются в процессе настройки на основе ёмкости вашего экземпляра и должны соответствовать потребностям большинства установок.

 Если у вас большой объём данных и мощная машина, просто запустите файл `openreplay -e` и переопределите ресурсы сервиса. В приведённом ниже примере мы делаем это для worker `http`. Но это можно сделать для любого сервиса (т. е. sink, storage, postgresql, redis и т. д.), раскомментировав и обновив приведённый ниже блок. Если вам нужно сделать это для более чем одного сервиса, просто скопируйте и переименуйте/обновите тот же блок (следите за дублированием).
...
1. Сделайте дамп БД в постоянном томе:
   ...
   Если вы используете обычную установку, выполните следующую команду, чтобы сделать дамп базы данных:

   > Примечание. Можно ожидать, что данные сожмутся в соотношении 3:1 (например: если у вас 15G данных postgres, то в дампе будет 5G). Поэтому убедитесь, что у вас достаточно места на диске.
