...
В противном случае, если ваше веб-приложение использует **Серверный рендеринг (SSR)** (например, NextJS, NuxtJS), используйте следующий код. Убедитесь, что `tracker.start()` вызывается после запуска приложения (в `useEffect` или `componentDidMount`).
