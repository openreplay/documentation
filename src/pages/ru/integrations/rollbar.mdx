...
Для того, чтобы OpenReplay связал запись в Rollbar с записанным сеансом пользователя, необходимо передать уникальный токен в качестве `extra_data` к каждой ошибке бэкенда, которую вы хотите отслеживать.
