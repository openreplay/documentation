...
Для получения дополнительной информации о создании Access ID и Access Key обратитесь к этой [документации](https://help.sumologic.com/Manage/Security/Access-Keys).
...
Для связи события SumoLogic с записанной пользовательской сессией необходимо распространить уникальный токен из вашего фронтенда на ваш бэкенд при каждом запросе, который вы хотите отслеживать. Это можно сделать с помощью пользовательского HTTP-заголовка. В приведенном ниже примере мы используем функцию `fetch`, чтобы отправить этот заголовок.
...
Для того, чтобы OpenReplay связала сообщение SumoLogic с записанной пользовательской сессией, необходимо распространить уникальный токен в каждой ошибке бэкенда, которую вы хотите отслеживать.
