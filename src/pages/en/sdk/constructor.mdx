...
<Aside type="tip">
If you're using a particular web framework, check out our [Tracker Setup](/sdk/using-or) section to know how to get started.
</Aside>
...
- `analytics?: { active: boolean, ingestPoint?: string }` Enables the [Analytics SDK](/sdk/analytics), which exposes `tracker.analytics` (events, super properties and user profiles) plus the top-level `tracker.track()` shorthand. The SDK is **not** instantiated unless `active` is `true`, and `tracker.analytics` stays `null` until then. `ingestPoint` lets self-hosted deployments send analytics to a different endpoint than the replay ingest; it is ignored on OpenReplay Cloud. Default: `{ active: false }`.
...
- `obscureInputNumbers?: boolean` Obscures numbers in input fields. Number values will be converted to a random chain of asterisks. If disabled, make sure to [sanitize input fields](/sdk/sanitize-data#granular-level) that may contain sensitive data such as zipe codes or credit card details. Default: `true`.
...
Note that excluded data is obscured or suppressed before sending the data to OpenReplay servers. Changes applied to the above options cannot be retroactive and will only apply to newly collected data. See [Sanitize Data](/sdk/sanitize-data) for more details.
...
<Aside type="caution">
    Note that when using `__DISABLE_SECURE_MODE` from your local workstation, you **might** see visual differences when watching the replay, like CSS styles not properly loading. Follow [this guide](/troubleshooting/localhost) to fix the situation.
</Aside>

## Methods

<a class="or-card2 or-card-wide" href="/sdk/methods"><span class="or-card-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m8 8-4 4 4 4M16 8l4 4-4 4M14 5l-4 14"/></svg></span><div><h4>Get to know the methods</h4></div></a>
