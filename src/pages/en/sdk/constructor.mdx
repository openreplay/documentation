...
- `inlineCss: number`: Forces the tracker to record and send remote css stylesheets as regular messages instead of caching them on backend, it can have the following values:
```
enum InlineCssMode {
