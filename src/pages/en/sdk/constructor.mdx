...
See [Network Options](/installation/network-options) for more details and examples. 
...
- `disableClickmaps?: boolean` disables selector calculation entirely 
- `minSelectorDepth?: number`  minimum length of an optimised selector (default 2). Example: `body > div > div > p => body > p`
- `nthThreshold?: number` how many selectors to try before falling back to nth-child selectors (default 1000). **Performance expensive operation, do not set higher than 2000**
- `maxOptimiseTries?: number` how many times it should try to optimise and shorten the selector (default 10 000).
