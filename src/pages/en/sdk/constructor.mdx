...
- `inlineCss: number`: Forces the tracker to record and send remote CSS stylesheets as regular messages instead of caching them on backend. This is useful when CSS files are private, and therefore cannot be cached by OpenReplay backend. Values can be:
    - `0`: (Disabled) The CSS file is cached by the backend. Default behavior.
    - `1`: (Inline) Will attempt to record the linked css file as AdoptedStyleSheet object.
    - `2`: (InlineFetched) Fetch the file, then simulate AdoptedStyleSheets behavior for the replay.
    - `3`: (PlainFetched) Fetch the file, then save it as plain css inside `<style>` node.
