...
- [coldStart()](/sdk/cold-start): Starts buffering messages (refreshing buffer) with possibility to call `start()` to activate recording and send the buffer
...
- [uploadOfflineRecording()](/sdk/upload-offline-recording): Uploads the stored session buffer to backend
- [forceFlushBatch()](/sdk/force-flush-batch): Force sends current batch of messages without waiting for its completion
