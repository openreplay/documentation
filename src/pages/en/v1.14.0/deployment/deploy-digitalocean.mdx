...
2. Choose the *Ubuntu Server 22.04 LTS* image (or higher)
3. Go for the 'Shared CPU' and pick the `Regular Intel | 4 CPUs | 8GB` plan. These are the minimum specs, otherwise OpenReplay backend services won't simply start. They're enough for a moderate volume, but if you're expecting high traffic, you should scale from here.
4. Click 'New SSH key' in 'Authentication' and follow the instructions to generate a key
5. Give your machine a name then hit 'Create droplet'
