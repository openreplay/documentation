...
5. Add the below claims in 'Attribute & Claims' block:
 - **tenantKey**: format `Basic` and set the value to <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/>:<ORDynamicCode id="port" initialValue={initialValues.port} client:idle/></code>, found in OpenReplay dashboard under 'Preferences' > 'Account'
 - **firstName**: format `Basic` and set the value to `user.givenname`
 - **lastName**: format `Basic` and set the value to `user.surname`
 - **internalId**: format `Basic` and set the value to `user.mail`
