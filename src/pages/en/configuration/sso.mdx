...
In you Identity Provider's dashboard, create a new app called `openreplay` (you can use this <a href="/public/static/OpenReplay.png" target="_blank">icon</a>), In the configuration page, make sure to set the following value (please make sure to replace `YOUR_DOMAIN` with the correct value, example `https://openreplay.mycompany.com`):
...
| idp_x509cert | The `x509cert`, must be a one-line string, without line breaks. You can use the <a href="https://www.samltool.com/format_x509cert.php" target="_blank">FORMAT A X509 CERTIFICATE tool</a> to format your value |
...
```
2. In your server, edit your `openreplay/scripts/helmcharts/vars.yaml`
3. Under the `chalice` section, add the following env variables:
   - **sp_crt**: must be a one-line string, without line breaks of the generated 'openreplay-sso.crt' file (you can use the <a href="https://www.samltool.com/format_x509cert.php" target="_blank">FORMAT A X509 CERTIFICATE tool</a> to format your value)
   - **sp_key**: must be a one-line string, without line breaks of the generated 'openreplay-sso.key' file
4. Save your changes and reinstall the web server:
```bash
cd openreplay/scripts/helmcharts && ./openreplay-cli -I
```
...
3. Set the 'App Name' to OpenReplay (you can upload this <a href="/public/static/OpenReplay.png" target="_blank">icon</a>) for your application) then press 'Next'
4. Set:
...
10. Under the `chalice` section, uncomment then set the following env variables:
   ...
   - **idp_x509cert**: X.509 Certificate, must be a one-line string, without line breaks (ou can use the <a href="https://www.samltool.com/format_x509cert.php" target="_blank">FORMAT A X509 CERTIFICATE tool</a> to format your value)
   - **idp_name**: Okta
...
3. Set the 'App name' to OpenReplay (you can upload this <a href="/public/static/OpenReplay.png" target="_blank">icon</a> for 'App icon'') then press 'Continue'
...
11. Under the `chalice > env` section, uncomment and set the following attributes using the values from step 4:
   ...
   - **idp_x509cert**: use the <a href="https://www.samltool.com/format_x509cert.php" target="_blank">FORMAT A X509 CERTIFICATE tool</a> to format the copied/downloaded certificate
...
3. Set the 'Display Label' to OpenReplay (you can upload this <a href="/public/static/OpenReplay150x150.png" target="_blank">icon</a> for your application) then move to 'SSO' tab
4. Set:
...
6. Define the below fields in 'CONSTANT ATTRIBUTES' by pressing the 'add attribute' button:
		- **tenantKey**: set the value to the one you got from OpenReplay dashboard under 'Preferences' > 'Account'
		- **role**: optional, the user’s role, should match role name already defined in OpenReplay (default = `member`)
		- **adminPrivileges**: optional, set the value to `true` if you want to give new users admin privilege, `false` if not
		(default = `false`)
...
10. Under the `chalice` section, uncomment then set the following env variables:
		...
		- **idp_x509cert**: the downloaded certificate, must be a one-line string, without line breaks (ou can use the <a href="https://www.samltool.com/format_x509cert.php" target="_blank">FORMAT A X509 CERTIFICATE tool</a> to format your value)
		- **idp_name**: JumpCloud
12. Finally, save your changes and reinstall the web server:
```bash
cd openreplay/scripts/helmcharts && ./openreplay-cli -I
