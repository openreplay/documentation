...
4. Finally, update OpenReplay services:

To update all `postgres` variables in `openreplay -e` and point them towards the external database:
...
Save and quit using `:wq` to reload the server.
