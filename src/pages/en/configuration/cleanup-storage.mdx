...
## Manual cleanup database (PostgeSQL)
...
Or you can manually cleanup your data by following the next sections.
...
### Cleanup specific sessions by time

If you want to clean all sessions, skip to [the next part](#cleanup-all-sessions) as it is faster and releases storage space instantly.
...
For OpenReplay Enterprise Edition:

```sql
--- Cascade delete all sessions and their related events captured before Jan 1st, 2021
DELETE FROM public.sessions WHERE start_ts < extract(epoch from '2021-01-01'::date) * 1000 AND session_id NOT IN (SELECT session_id FROM user_favorite_sessions);
```
PS: after running the previous query, the database will not release the storage space immediately, as it will schedule a cleaning for later, to manually force it to release storage, you can run the following queries:

<Aside type="note">
	These queries will lock each table so it will not be available, and it might take hours to finish
	according to your database, if you still have enough free storage space, you can leave the cleaning for the PostgreSQL automatic Vacuum.
	
	In addition it will require some free storage in order for it to work, that's why we highly recommend running these queries in the ascending order of [tables size](#tables-size)
</Aside>
