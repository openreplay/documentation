...
If you wish to enable http to https redirection (recommended), then uncomment the below block, under the `ingress-nginx` section, in `/var/lib/openreplay/vars.yaml`:
...
1. Open `/var/lib/openreplay/vars.yaml` and add your CSP in the `frontend` block. Make sure to update 
...
Cross-domain requests are allowed, by default, from all origins (`Access-Control-Allow-Origin: *`). If you wish to restrict recordings from few domains only, then open `/var/lib/openreplay/vars.yaml` and update the `http` block as below:
