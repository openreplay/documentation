...
To enable SMTP, edit `/var/lib/openreplay/vars.yaml` and update the below env variables in `email` section:
...
6. Use the displayed SMTP settings and credentials to configure SMTP in OpenReplay. Edit `/var/lib/openreplay/vars.yaml` and update the below env variables in `email` section:
