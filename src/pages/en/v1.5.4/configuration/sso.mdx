...
In you Identity Provider's dashboard, create a new app called `openreplay` (you can use this [icon](/static/OpenReplay.png)), In the configuration page, make sure to set the following value (please make sure to replace `YOUR_DOMAIN` with the correct value, example `https://openreplay.mycompany.com`):
...
3. Set the 'App Name' to OpenReplay (you can upload this [icon](/static/OpenReplay.png) for your application) then press 'Next'
4. Set:
