...
`user_id` must be a non-empty string — anything else throws.

`options.fromTracker` is used internally to avoid double-calling the tracker-level id hook; you typically don’t need it. `tracker.setUserID()` already calls `identify` for you.
...
The [default properties](/sdk/analytics/events#reserved-keys) collected by the SDK are ignored for the SDK’s local cache.

The key/value form requires both arguments to be truthy — `setProperties("plan", "")` or `setProperties("seats", 0)` throws `invalid user properties provided to set`. Use the object form for falsy values.
