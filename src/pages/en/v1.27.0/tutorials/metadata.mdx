...
In other words, if the user clicks on a specific link, or performs a particular action, that’s not metadata, that’s a custom event (check out [the docs about Custom Events](/product-analytics/custom-events) if you want to know more about them).
