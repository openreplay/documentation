...
- `canvasEnabled: boolean`: Controls whether canvas recording is enabled or not. Defaults to `false`.
- `canvasQuality: 'low' | 'medium' | 'high'`: Sets the quality of the recorded snapshots. Defaults to `medium`.
- `canvasFps: number`: Defines the number of frames per second at which the canvas elements are recorded. Defaults to `2`.
