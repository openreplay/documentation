...
- `useProxy`: Switches tracker from patching window.fetch/window.XMLRequest to using proxy objects (will be the default method in future updates). Default: `false`.
...
- `axiosInstances`: For specifying the custom axios instances to include in tracking.
