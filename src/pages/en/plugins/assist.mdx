...
By default, assist connects to the server as soon as the tracker starts. The `autostart` option, together with the `Autostart` enum (exported by the plugin), lets you control this behavior:

```js
import trackerAssist, { Autostart } from '@openreplay/tracker-assist';

tracker.use(trackerAssist({ autostart: Autostart.Disabled }));
```

- `Autostart.Auto` (default): assist always connects on tracker start.
- `Autostart.Disabled`: assist never connects automatically; it starts only via an explicit [`start()`](#methods) call.
- `Autostart.Continuation`: assist starts only via [`start()`](#methods), but the choice is remembered in `sessionStorage` so it auto-continues across page reloads until [`stop()`](#methods) is called.

### Session view confirmation

Set `requestConfirm: true` to require the user's permission before an agent can view their session. Assist starts and connects as usual, but nothing is sent to agents until the user approves the confirmation popup, which is shown as soon as the first agent connects (default text: "Agent requested to view this session live. Allow?"). Once approved, tracking restarts so agents receive a full snapshot of the page.
...
The popup's text and style can be customized via the `sessionConfirm` option, which takes the same `ConfirmOptions` object as `callConfirm` and `controlConfirm` (see [Remote Control Widget](#remote-control-widget)).
