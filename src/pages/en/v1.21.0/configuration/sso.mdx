...
Single-Sign-On (via SAML2) is available on OpenReplay Enterprise Edition and Dedicated instances only.
...
| Field | Value |
|----------|-------------|
| tenantKey | <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> its value can be found in the Dashboard, under 'Preferences' > 'Account' (Note: It should be ignored/left empty if you're using a dedicated/cloud instance) |
...
5. Define the below fields in 'Attribute Statements':
 - **tenantKey**: format `Basic` and set the value to <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, found in OpenReplay dashboard under 'Preferences' > 'Account' (Note: It should be ignored/left empty if you're using a dedicated/cloud instance).
...
5. Add the below claims in 'Attribute & Claims' block and ensure the 'Namespace' field is left empty for each of them:
 - **tenantKey**: set the value to <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code>, found in OpenReplay dashboard under 'Preferences' > 'Account' (Note: It should be ignored/left empty if you're using a dedicated/cloud instance).
...
11. Under the `chalice > env` section, uncomment and set the following attributes using the values from step 4:
   ...
   - **idp_tenantKey**: <code><ORDynamicCode id="tenantKey" initialValue={initialValues.tenantKey} client:idle/></code> found in OpenReplay dashboard under 'Preferences' > 'Account' (Note: It should be ignored/left empty if you're using a dedicated/cloud instance).
