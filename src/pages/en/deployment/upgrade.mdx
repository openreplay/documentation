...
2. Clone the new OpenReplay version and update the CLI:
   
  ```bash
	# Clone the last available version
...
3. Upgrade OpenReplay but make sure to replace the `/path/to/old_vars.yaml` with the correct one (example: `openreplay_v1.9.0/`):
...
5. Update `fromVersion` variable in `/openreplay/scripts/helmcharts/vars.yaml` to reflect the new version. As an example if you're moving from `v1.9.0` to `v1.10.0` then update the `fromVersion` like below:
