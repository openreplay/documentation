...
If you would like to go further in configuring the tracker, check [JavaScript SDK](/sdk/constructor#initialization-options) for the list of available options.
...
<div class="or-rowlist">
  <LinkRow href="/session-replay/identify-user" title="Identify a User" icon="user" desc="Associate your internal user ID with the session being recorded." />
  <LinkRow href="/session-replay/metadata" title="Metadata" icon="metadata" desc="Also referred to as traits or user variables, metadata provides additional information about users while recording sessions." />
  <LinkRow href="/product-analytics/custom-events" title="Custom Events" icon="api" desc="Add more intelligence by recording domain-specific events alongside session replays." />
  <LinkRow href="/session-replay/error-reporting" title="Error Reporting" icon="error" desc="Report errors and exceptions and see them under the ERRORS tab." />
  <LinkRow href="/deployment/upload-sourcemaps" title="Upload Source-Maps" icon="highlight" desc="See the source code context obtained from stack traces in their original form." />
