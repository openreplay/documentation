...
fetch('/api/endpoint', {
  ...
  headers: {
    ...
    'openReplaySession.id': openReplay.getSessionToken(), // or window.OpenReplay.getSessionToken() if using the snippet
  },
  // ...other options
})
  .then(response => {
...
1. Go to **Integrations** in OpenReplay.
2. Select the **Backend Logging** tab.
3. Select the project to which you want to enable the Dynatrace integration: Locate the Dynatrace integration card > Click on **Enable**.
