...
تُرفَض مجموعتان مختلفتان من المفاتيح، بحسب الدالة المساعِدة:

**الخصائص الافتراضية** — تجمعها حزمة تطوير البرمجيات (SDK) بنفسها وتُرسَل مع كل حدث. تتخطّاها `setProperty` بصمت، لذا لا يمكنك حجبها بخاصية فائقة:
...
**مفاتيح الحمولة** — `properties` و`token` و`timestamp`. وهي جزء من غلاف الحدث (envelope)، لذا تتخطّاها `setPropertiesOnce` و`unsetProperties`.

<Aside type="note">
	تُطبَّق القائمتان عبر دوال مختلفة: تحمي `setProperty` الخصائص الافتراضية، بينما تحمي `setPropertiesOnce` و`unsetProperties` مفاتيح الحمولة. وينطبق التقسيم نفسه على وحدة People، حيث تحمي كلٌّ من `setProperties` و`setPropertiesOnce` و`appendValues` الخصائص الافتراضية.
</Aside>
...
- يتم تجاهل [الخصائص الافتراضية](#المفاتيح-المحجوزة) التي تجمعها حزمة تطوير البرمجيات ولن تُضاف كخصائص فائقة.
- يتم تجاهل صيغة المفتاح/القيمة عندما تكون `value` مساوية لـ `undefined`؛ مرّر `null` لتخزين قيمة فارغة.
