...
<Aside type="tip">
إذا كنت تستخدم إطار عمل ويب معيناً، فاطلع على قسم [Tracker Setup](/sdk/using-or) الخاص بنا لمعرفة كيفية البدء.
</Aside>
...
- `analytics?: { active: boolean, ingestPoint?: string }` يُفعّل [حزمة تطوير التحليلات (Analytics SDK)](/sdk/analytics)، التي تُتيح `tracker.analytics` (الأحداث والخصائص الفائقة وملفات تعريف المستخدمين) إضافةً إلى الاختصار `tracker.track()` على المستوى الأعلى. **لا** تُنشأ حزمة التطوير ما لم تكن قيمة `active` هي `true`، وتظل `tracker.analytics` مساوية لـ `null` حتى ذلك الحين. يتيح `ingestPoint` لعمليات النشر ذاتية الاستضافة إرسال التحليلات إلى نقطة نهاية مختلفة عن نقطة استيعاب إعادة التشغيل؛ ويتم تجاهله على OpenReplay Cloud. القيمة الافتراضية: `{ active: false }`.
...
- `obscureInputNumbers?: boolean` يخفي الأرقام في حقول الإدخال. سيتم تحويل القيم الرقمية إلى سلسلة عشوائية من العلامات النجمية. في حال تعطيله، تأكد من [تنقية حقول الإدخال](/sdk/sanitize-data#المستوى-الدقيق) التي قد تحتوي على بيانات حساسة مثل الرموز البريدية أو تفاصيل بطاقة الائتمان. القيمة الافتراضية: `true`.
...
لاحظ أن البيانات المستبعَدة تُخفى أو تُحجب قبل إرسال البيانات إلى خوادم OpenReplay. لا يمكن أن تكون التغييرات المطبَّقة على الخيارات المذكورة أعلاه بأثر رجعي وستنطبق فقط على البيانات المجموعة حديثاً. راجع [Sanitize Data](/sdk/sanitize-data) لمزيد من التفاصيل.
...
<a class="or-card2 or-card-wide" href="/sdk/methods"><span class="or-card-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m8 8-4 4 4 4M16 8l4 4-4 4M14 5l-4 14"/></svg></span><div><h4>تعرّف على الطرق</h4></div></a>
