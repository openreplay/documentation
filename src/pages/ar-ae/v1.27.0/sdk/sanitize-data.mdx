...
استخدم خيار `sanitizer` لحذف البيانات الحساسة من طلب/استجابة Fetch/XHR. راجع [خيارات الشبكة](/sdk/network-options#الخيارات) لمزيد من التفاصيل.

تحقق أيضًا من هذا الدرس خطوة بخطوة حول كيفية [التقاط طلبات الشبكة وتنقيتها](/tutorials/capture-request).

#### GraphQL

لتنقية بيانات GraphQL، تحتاج إلى نسخ المتغيرات الرئيسية ونتائج العملية قبل استدعاء الـ tracker. اطّلع على كيفية [تنقية البيانات المُسجَّلة في GraphQL](/tutorials/graphql/#تنقية-البيانات-المسجَّلة) لمزيد من التفاصيل.
...
استخدم خيارات `actionFilter` و`actionTransformer` و`stateTransformer` لحذف الإجراءات والأشجار الفرعية الحساسة من المخزن (store) لديك. راجع [توثيق إضافة redux](/plugins/redux/#الخيارات) لمزيد من التفاصيل.

#### VueX

استخدم خيارات `filter` و`transformer` و`mutationTransformer` لحذف الطفرات (mutations) والأشجار الفرعية الحساسة من المخزن (store) لديك. راجع [توثيق إضافة vuex](/plugins/vuex/#الخيارات) لمزيد من التفاصيل.

#### Zustand

استخدم خيارات `filter` و`transformer` و`mutationTransformer` لحذف الطفرات (mutations) والأشجار الفرعية الحساسة من المخزن (store) لديك. راجع [توثيق إضافة zustand](/plugins/zustand/#الخيارات) لمزيد من التفاصيل.

#### Pinia

استخدم خيارات `filter` و`transformer` و`mutationTransformer` لحذف الطفرات (mutations) والأشجار الفرعية الحساسة من المخزن (store) لديك. راجع [توثيق إضافة pinia](/plugins/pinia/#الخيارات) لمزيد من التفاصيل.

#### NgRx

استخدم خيارات `actionFilter` و`actionTransformer` و`stateTransformer` لحذف الإجراءات والأشجار الفرعية الحساسة من المخزن (store) لديك. راجع [توثيق إضافة ngrx](/plugins/ngrx/#الخيارات) لمزيد من التفاصيل.
