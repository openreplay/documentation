...
<Aside type="tip">
  لاحظ أنه إذا كان لديك مشاريع متعددة، فيجب أن تنتبه إلى مفتاح المشروع الذي تختاره.
</Aside>
...
بخلاف ذلك، إذا كان تطبيق الويب الخاص بك **معروضًا من جانب الخادم (SSR)** (مثل NextJS، NuxtJS) استخدم الكود أدناه. تأكد من أن `tracker.start()` يتم استدعاؤه بمجرد بدء التطبيق (في `useEffect` أو `componentDidMount`).
