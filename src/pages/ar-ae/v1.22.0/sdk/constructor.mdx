...
<Aside type="tip">
  لاحظ أنه إذا كان لديك مشاريع متعددة، فيجب أن تكون حذرًا في اختيار مفتاح المشروع الذي تختاره.
</Aside>
...
خلاف ذلك، إذا كان تطبيق الويب الخاص بك **معروضًا من جانب الخادم (SSR)** (مثل NextJS، NuxtJS) استخدم المقتطف أدناه.
...
<Aside type="tip">
إذا كنت تستخدم إطار عمل ويب معينًا، تحقق من قسم [Tracker Setup](/using-or) لمعرفة كيفية البدء.
</Aside>
...
- `sessionHash?: string` تجزئة (hash) الجلسة الأولية. هذا مفيد عندما تنتقل الجلسات عبر نطاقات فرعية مختلفة في تطبيق الويب الخاص بك ولكنك تريد دمجها في تسجيل واحد. في حالة عدم إمكانية متابعة الجلسة (غير موجودة أو منتهية)، سيبدأ المتتبع تلقائيًا جلسة جديدة. كما يتم إرجاعه عند `stop()`. مزيد من التفاصيل حول هذا [هنا](/troubleshooting/session-recordings#replaysarebrokenacrosssubdomains).
...
- `obscureInputNumbers?: boolean` يخفي الأرقام في حقول الإدخال. سيتم تحويل القيم الرقمية إلى سلسلة عشوائية من النجوم. في حال تعطيله، تأكد من [تنظيف حقول الإدخال](/installation/sanitize-data/#granular-level) التي قد تحتوي على بيانات حساسة مثل الرموز البريدية أو تفاصيل بطاقات الائتمان. الافتراضي: `true`.
