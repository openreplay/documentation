...
يجب أن يكون `user_id` سلسلة نصية غير فارغة — وأي قيمة أخرى تؤدي إلى رمي خطأ.

تُستخدم `options.fromTracker` داخليًا لتجنّب الاستدعاء المزدوج لخطّاف المعرّف على مستوى أداة التتبّع؛ وعادةً لن تحتاج إليها. إذ تستدعي `tracker.setUserID()` الدالة `identify` نيابةً عنك بالفعل.
...
تُتجاهَل [الخصائص الافتراضية](/sdk/analytics/events#المفاتيح-المحجوزة) التي تجمعها حزمة SDK بالنسبة إلى الذاكرة المؤقتة المحلية لحزمة SDK.

تتطلّب صيغة المفتاح/القيمة أن تكون قيمة كلا الوسيطين صحيحة (truthy) — إذ يؤدي `setProperties("plan", "")` أو `setProperties("seats", 0)` إلى رمي الخطأ `invalid user properties provided to set`. استخدم صيغة الكائن للقيم الخاطئة (falsy).
