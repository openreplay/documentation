...
<Aside type="tip">
	这是一项实验性功能，其实现方式未来可能会发生变化。OpenReplay 无法在本地运行，因此即使使用 Docker，你仍然需要一台服务器。请考虑我们的其他[部署选项](/deployment)。
</Aside>
...
4. 选择你的实例类型。最低规格为 `2 vCPUs, 8 GB of RAM, 50 GB of storage`，并且需要 x86 架构，否则 OpenReplay 后端服务将根本无法启动。因此，我们建议**至少**使用 `t3.large`（或同等配置），这足以应对低/中等流量。如果你预期会有高流量，则应在此基础上进行扩展。
