...
您也可以[从源代码](/deployment/deploy-source)部署。尽管这种方法需要更多努力，但它提供了很大的灵活性，因为您可以使用最新的代码并在需要时进行自定义。
...
<ORCard url="/deployment/openreplay-admin" title="管理 OpenReplay"   description="通过直接访问日志、重新安装服务、增加容量等方式轻松管理您的实例。" icon="administer" width="50"/>
<ORCard url="/deployment/upgrade" title="升级部署"   description="通过将 OpenReplay 实例升级到最新版本来确保其保持最新状态。" icon="upgrade" width="50"/>
</div>