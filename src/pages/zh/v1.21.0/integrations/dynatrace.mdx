...
了解如何将 [Dynatrace](https://www.dynatrace.com/) 后端日志与 [OpenReplay](https://www.openreplay.com/) 会话回放集成，以扩展你的监控和调试能力。
...
> **参考**：详细步骤请参阅 Dynatrace 文档：[Log custom attributes](https://docs.dynatrace.com/docs/observe-and-explore/logs/lma-analysis/logs-and-events/lma-log-custom-attributes)。
...
2. **Client ID** 和 **Client Secret**：在第一步中保存的内容。
...
![OpenReplay 中的 Dynatrace 集成](/static/dynatrace-openreplay.png#center)
...
![OpenReplay 中的 Dynatrace 追踪](/static/traces-dynatrace.png#center)

> 类型为 `LOG` 且状态为 `WARN`、`ERROR`、`INFO` 和 `NONE` 的日志通过 Dynatrace Log Monitoring API v2 检索。
