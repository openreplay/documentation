...
1. 前往 **[Datadog > Integrations > APIs](https://app.datadoghq.com/account/settings#api)**，生成 API Key，或使用现有的 API Key。
...
3. 选择你想要为其启用 Datadog 集成的项目：找到 Datadog 集成卡片 > 点击它。
...
要将 Datadog 事件与录制的用户会话关联起来，需要在你想要跟踪的每个请求中，将唯一的会话 ID `openReplaySession.id` 从前端传递到后端。这可以通过自定义 HTTP 标头来实现。
...
下面是一个使用 logging 模块的 Python 示例，可在你的日志中自动包含 openReplaySession.id。
