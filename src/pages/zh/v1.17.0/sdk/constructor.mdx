...
以下命令将为您安装追踪器及其 SDK，让您可以利用所有追踪器功能。
...
- `forceSingleTab: boolean` 禁用将跨多个标签进行的用户会话缝合成单个重放的多标签录制功能。相反，启用此选项后，在不同浏览器标签中捕获的会话将被录制为单独的录制。默认值：`false`。
- `disableStringDict: boolean` 禁用录制中的字符串字典逻辑（用于优化存储）。默认值：`false`。
- `fixedCanvasScaling: boolean` 强制以 1 的像素密度而非 devicePixelRatio 渲染画布图像。（降低图像的潜在质量，以使最终结果的体积更小）
...
请参见[网络选项](/installation/network-options)获取示例和更多关于如何屏蔽敏感数据的详细信息。 
