...
title: "会话录制问题"
metaTitle: "会话录制问题"
metaDescription: "各种阻止回放被捕获或正确显示的问题。"
---
...
OpenReplay 需要公共访问权限来复制你应用的部分资源（CSS、字体和图标），以便正确渲染录制。你的站点还必须使用 SSL/HTTPS，否则 tracker 将无法启动。由于这些原因，除非你应用此[变通方法](/troubleshooting/localhost)，否则在 localhost 中将不会捕获会话。
...
如果你看到类似下面的错误，则说明 OpenReplay 因缺少内容安全策略而无法启动。请参阅[此处](/troubleshooting/csp)了解需要添加的策略。
...
OpenReplay 需要访问诸如你的 css 之类的资源才能使回放正常工作。事实上，这些文件会被我们的后端复制并缓存，因此即使你的 Web 应用发生了变化，你也能查看旧的录制。这就是为什么你必须确保你的样式（以及图标和字体）是可公开访问的。如果你的资源托管在受限域名（私有或受保护）上，那么 OpenReplay 将无法复制并在回放中使用它们。你可以将它们复制到可公开访问的域名，然后使用 `resourceBaseHref` 选项在 [tracker 的构造函数](/sdk/constructor)中指定该域名。

如果你的同源 iFrame 含有嵌套框架，请确保在每个嵌入的框架中使用绝对路径（在指向你的 CSS 文件的 `<link>` 元素中），或使用 `resourceBaseHref` 选项在 [tracker 的构造函数](/sdk/constructor)中指定该域名。

如果你在 localhost 中进行测试，样式将不会被渲染，因为它们位于你的机器上，OpenReplay 无法访问。请按照[本指南](/troubleshooting/localhost)来解决该问题。
