...
- `forceSingleTab: boolean` 禁用多标签录制功能，该功能将跨多个标签进行的用户会话缝合成单条重放。相反，启用此选项后，在不同浏览器标签中捕获的会话将记录到单独的录制中。默认值：`false`。
- `disableStringDict: boolean` 禁用录制中的字符串字典逻辑（用于优化存储）。默认值：`false`。
- `fixedCanvasScaling: boolean` 强制 canvas 图像以像素密度 1 渲染，而不是 devicePixelRatio。（降低图像的潜在质量，以使最终结果在大小上更小）。默认值：`false`。
- `disableCanvas?: boolean` 禁用 canvas 元素的录制。默认值：`false`。
- `assistSocketHost?: string` 可用于为 assist 插件连接设置特定主机。默认值为 `ingestPoint` 的值。
...
`crossdomain` 选项与捕获来自不同域名的 iFrames 有关。它与 `captureIFrames` 选项一起使用，该选项必须设置为 `true`。
...
请参见[跨域 iFrame 跟踪](tutorials/crossdomain)了解有关如何从不同域名捕获 iFrame 的更多详细信息。
...
- `onFlagsLoad` 回调，用于在 feature flags 加载完成后执行操作（每次）。
