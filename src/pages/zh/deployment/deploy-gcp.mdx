...
4. 选择你的机器类型。最低配置要求为 `2 vCPUs, 8 GB of RAM, 50 GB of storage`，并基于 x86 架构，否则 OpenReplay 后端服务根本无法启动。因此，我们建议**至少**使用 `n2-standard-2`（或同等配置——请注意，已知 `e2-standard-2` 在部署期间会出现问题），它足以应对低/中等流量。如果你预期会有高流量，则应在此基础上进行扩展。
