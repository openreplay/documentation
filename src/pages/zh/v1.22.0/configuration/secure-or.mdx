...
强烈建议**仅允许通过开放的 80/443 端口访问该机器**，并使用安全组实现。有些组件会监听 0.0.0.0 地址以简化网络配置，例如 k3s 服务器 API、flannel API 等。
...
为了指示浏览器是否应允许在 `<frame>, <iframe>, <embed>` 或 `<object>` 中呈现页面，并避免点击劫持（click-jacking）攻击，请使用 `openreplay -e` 编辑配置来设置 `X-Frame-Options` 响应头：
...
5. 运行 `openreplay -e`，并在 `frontend` 代码块中指定您新构建的前端镜像：
