...
如果一切顺利，你的 Docker 镜像仓库中应该会有一份从你拥有的源代码构建的镜像列表，可供安装。
镜像列表如下：
...
只需为你 docker 镜像仓库中的每个镜像添加一个部分，包含以下信息：
...
请注意，任何你未在 `vars.yaml` 文件中引用的镜像都将从 OpenReplay 的**官方仓库**部署和安装，这意味着你将无法获得从你的源代码构建的版本。
...
3. 如果你希望启用 http 到 https 的重定向（推荐），请在 `vars.yaml` 中的 `ingress-nginx` 部分下取消注释以下代码块：
