...
`user_id` 必须是非空字符串——其他任何值都会抛出错误。

`options.fromTracker` 在内部使用，以避免重复调用追踪器层级的 id 钩子；通常你不需要用到它。`tracker.setUserID()` 已经会为你调用 `identify`。
...
对于 SDK 的本地缓存，SDK 收集的[默认属性](/sdk/analytics/events#保留键)会被忽略。

键/值形式要求两个参数都为真值（truthy）——`setProperties("plan", "")` 或 `setProperties("seats", 0)` 会抛出 `invalid user properties provided to set` 错误。对于假值（falsy），请使用对象形式。
